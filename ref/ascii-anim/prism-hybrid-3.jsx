  ...
  const renderFrame = useCallback(() => {
    ...
    for (let row = 0; row < rows; row++) {
      ...
      for (let col = 0; col < cols; col++) {
        ...
        // Two layers: a sharp bright edge + a wider soft bloom
        let prismGlow = 0;
        let prismBloom = 0;
        for (let e = 0; e < 3; e++) {
          ...
          // Sharp inner edge
          prismGlow = Math.max(prismGlow, Math.max(0, 1.0 - d * 120) * 0.55);
          // Wider soft bloom
          prismBloom = Math.max(prismBloom, Math.max(0, 1.0 - d * 30) * 0.18);
        }
        const totalPrism = prismGlow + prismBloom;
    ...
    // ── Prism edge characters ──
    // Draw faint characters along the triangle edges
    ctx.font = `${size}px ${fontFamily}`;
    const edgeChars = ["╱", "│", "╲", "─", "△"];
    for (let e = 0; e < 3; e++) {
      const a = prismVerts[e];
      const bv = prismVerts[(e + 1) % 3];
      const steps = Math.ceil(Math.max(Math.abs(bv.x - a.x) * cols, Math.abs(bv.y - a.y) * rows) * 1.5);
      for (let i = 0; i <= steps; i++) {
        const t = i / steps;
        const ex = a.x + (bv.x - a.x) * t;
        const ey = a.y + (bv.y - a.y) * t;
        const ec = Math.floor(ex * cols);
        const er = Math.floor(ey * rows);
        if (ec >= 0 && ec < cols && er >= 0 && er < rows) {
          // Pick edge character based on edge direction
          const edgeDx = bv.x - a.x, edgeDy = bv.y - a.y;
          const angle = Math.atan2(edgeDy, edgeDx);
          let ch;
          if (Math.abs(angle) < 0.4) ch = "─";
          else if (angle > 0.4 && angle < 1.2) ch = "╲";
          else if (angle > 1.2) ch = "│";
          else if (angle < -0.4 && angle > -1.2) ch = "╱";
          else ch = "│";
          ctx.fillStyle = `rgb(${bgR + 45},${bgG + 40},${bgB + 70})`;
          ctx.fillText(ch, ec * charW, er * charH);
        }
      }
    }
    // Vertex markers
    ctx.fillStyle = `rgb(${bgR + 60},${bgG + 55},${bgB + 85})`;
