...
func (m *Model) View() string {
	...
	// starting at terminal background and increasing brightness by +1 RGB per row.
	// Each row shows: █ #RRGGBB
	// {
	...
	//
	// 		// Stamp hex code beside the block: " #RRGGBB"
	...
	// Phase 11: Render grid to ANSI string
	return m.renderGrid()
}
...
func (m *Model) renderGrid() string {
	...
	for y := 0; y < renderHeight; y++ {
		...
		// Reset SGR at line start — each line is self-contained so
		// truncation of one line can't cascade corruption to the next.
		...
		for x := 0; x < m.Width; x++ {
			c := m.grid[rowOff+x]
			if firstInLine || c.r != lastR || c.g != lastG || c.b != lastB {
	...
	b.WriteByte('\n')
	b.WriteString("\x1b[38;2;51;51;80m  press any key to continue\x1b[0m\x1b(B")
