# Prism - Complete Documentation v2.4.1
...
### Part V — Monorepo Architecture (v2.4.1)
...
69. [Documentation Site Overview](#documentation-site-overview)
...
| **Electron Desktop App** | `cmd/prism-electron/` | TypeScript, React 18, Electron 40, Vite | Standalone desktop app, V2 IDE shell, native menus |

All three share the same `.prism/` directory structure, `stories.json` schema, signal protocol, and Claude CLI integration. They can be used independently or side-by-side. The Electron app features a full V2 IDE shell with activity bars, collapsible rails, tabbed editors, and floating chat pill. It shares all business logic, React UI components, and the gRPC-over-postMessage protocol with the VS Code extension via a proper npm monorepo with `packages/prism-core` and `packages/prism-ui` shared packages (see Part V — Monorepo Architecture). A VitePress documentation site at `prism-docs/` provides navigable, searchable documentation across ~75 pages (see Part VI).
...
│ ─────────────────────────────────────────────────────────────────────────── │
│ [14:32:05] INFO  Prism CLI v2.4.1                                          │
...
```
 IMPLEMENT ╲ ⚡ Spectrum ╲  main ╲ STORY-003 ╲                ╱ v2.4.1 ╱ 3✓ 0✗ ╱ 8/12 ╱ iter 3 ╱ 🕒 2m 15s
```
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                     Prism VS Code Extension v2.4.1                          │
├──────────────┬──────────────┬──────────────┬───────────────┬────────────────┤
...
- **V2 IDE shell** with ActivityBar, ContentRail, TabBar, BottomPanel, and BottomStatusBar (v2.4.1)
...
The repository was restructured from two independent applications with fragile path aliases into a proper npm workspaces monorepo in v2.3.5, with continued refinements through v2.4.1.
...
The monolithic documentation file was split into a navigable VitePress site at `prism-docs/` in v2.4.1. The site provides the same content organized across ~75 pages with full-text search, syntax highlighting, and spectral-themed styling.
