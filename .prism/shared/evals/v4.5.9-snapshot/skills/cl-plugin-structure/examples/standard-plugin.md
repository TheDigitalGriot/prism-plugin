...
> **Note:** The PreToolUse hook uses `command` type (not `prompt`) because linting is a deterministic check — no LLM evaluation needed. This saves ~1,000 tokens per Write/Edit call.
...
### .claude-plugin/marketplace.json (if distributing via marketplace)

```json
{
  "name": "code-quality-marketplace",
  "metadata": {
    "description": "Code quality tools for Claude Code"
  },
  "owner": {
    "name": "Quality Team",
    "email": "quality@example.com"
...
> **Note:** `description` goes inside `metadata` at the marketplace root level, NOT as a direct root field. The plugin-level `description` goes directly in each plugins array entry. These are different schemas — don't confuse them.

## Validation

Always validate after creating or modifying plugin files:

```bash
claude plugin validate .
```

This checks plugin.json, marketplace.json, frontmatter in skills/agents/commands, and hooks.json. A plugin that fails validation will silently not load.
...
6. **Validated**: Always run `claude plugin validate .` before distribution
